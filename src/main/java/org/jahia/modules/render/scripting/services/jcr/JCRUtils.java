package org.jahia.modules.render.scripting.services.jcr;
...
public class JCRUtils {
    ...
    /**
     * Returns the first parent of the specified node, which has the ACL inheritance broken. If not found,
     * null<code>null</code> is
     * returned.
     *
     * @param node the node to search parent for
     * @return the first parent of the specified node, which has the ACL inheritance broken. If not found,
    ...
    /**
     * Returns <code>true</code> if the current node has the specified type or at least one of the specified node
     * types.
     *
     ...
     * empty list.
     *
     * @param node the current node to start the lookup from
     * @param type the required type of the parent node(s)
    ...
    /**
     * Returns the first parent of the current node that has the specified node type. If no matching node is found,
     * <code>null</code> is
     * returned.
     *
     * @param node the current node to start the lookup from
     * @param type the required type of the parent node
     * @return the first parent of the current node that has the specified node type. If no matching node is found,
     * <code>null</code> is
     * returned
     */
    ...
    /**
     * Returns a string with comma-separated keywords, found on the current node (or the parent one, if inheritance is
     * considered), or an
     * empty string if no keywords are present.
     *
     * @param node             the node to retrieve keywords from
     * @param considerInherted if set to <code>true</code> the keywords are also looked up to the parent nodes, if not
     *                         found on the current one
     * @return a string with comma-separated keywords, found on the current node (or the parent one, if inheritance is
     * considered), or an
     * empty string if no keywords are present
     */
